...
import { Calendar, MapPin, User, Clock, CheckCircle, XCircle, Eye, CheckCircle2, Ban, Clock as ClockIcon, Search, CalendarDays } from "lucide-react";
import { PageSkeleton } from "@/components/ui/loading";
import { useAllEvents, useUpdateEventApproval, type PendingEvent } from "@/lib/hooks/use-admin-data";
import { EventDetailModal } from "@/components/events/event-detail-modal";
import { toast } from "sonner";

// Event Card Component
const EventCard = ({ event, onApprove, onReject, isProcessing, onClick }: {
  event: PendingEvent;
  ...
  const getStatusBadge = () => {
    ...
    } else if (event.is_approved === true) {
      return (
  ...
  return (
    <Card className="h-full flex flex-col overflow-hidden cursor-pointer hover:shadow-lg transition-shadow duration-200" onClick={onClick}>
      ...
      <CardContent className="flex-1 flex flex-col min-h-0">
        ...
        <div className="space-y-3 flex-1 min-h-0">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center gap-2 text-gray-600">
              ...
              <span className="truncate">Capacity: {event.capacity || 'TBD'}</span>
            </div>
          </div>
        </div>
        ...
        {event.is_approved === null && (
          <div className="flex gap-2 mt-4 pt-4 border-t flex-shrink-0" onClick={(e) => e.stopPropagation()}>
            <Button
...
export default function EventsPendingPage() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    switch (activeTab) {
      case 'pending':
        filtered = filtered.filter(event => event.is_approved === null);
        break;
      case 'approved':
        filtered = filtered.filter(event => event.is_approved === true);
        break;
      case 'rejected':
        filtered = filtered.filter(event => event.is_approved === false);
        break;
    }
  ...
  const tabs = useMemo(() => [
    {
      ...
      count: allEvents?.filter(event => event.is_approved === null).length || 0
    },
    {
      id: 'approved',
      label: 'Approved',
      count: allEvents?.filter(event => event.is_approved === true).length || 0
    },
    {
      id: 'rejected',
      label: 'Rejected',
      count: allEvents?.filter(event => event.is_approved === false).length || 0
    }
  ], [allEvents]);
  ...
  const handleApprove = async (eventId: string) => {
    ...
    try {
      await updateApproval.mutateAsync({ eventId, isApproved: true });
  ...
  const handleReject = async (eventId: string) => {
    setProcessingEventId(eventId);
    try {
      await updateApproval.mutateAsync({ eventId, isApproved: false });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Review and manage all event submissions. {allEvents?.length || 0} total events.
        </p>
      </div>
      ...
      <Tabs
        tabs={tabs}
        activeTab={activeTab}
        onTabChange={(tabId) => setActiveTab(tabId as 'pending' | 'approved' | 'rejected')}
      />
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="mt-4 text-sm text-muted-foreground">
            ...
            {(searchQuery.trim() || selectedCategory !== "all" || startDate || endDate) && (
              <span className="ml-2">
                (filtered by {[
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-500">
              {(() => {
                if (searchQuery.trim() || selectedCategory !== "all" || startDate || endDate) {
