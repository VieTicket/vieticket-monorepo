...
class SimpleAIService {
  ...
  async getPersonalizedRecommendations(
    ...
    try {
      ...
      console.log(`👤 User has ${userBehavior.searchQueries.length} search queries, ${userBehavior.viewedEvents.length} viewed events`);
      console.log('────────────────────────────────────────');
      ...
      console.log(`   🎯 Score Range: ${((maxScore - minScore) * 100).toFixed(1)}% spread`);
      console.log('────────────────────────────────────────');
      ...
      sortedRecommendations.forEach((rec, index) => {
        ...
        console.log(`   💰 Price: ${rec.event.price ? rec.event.price.toLocaleString() + 'đ' : 'Free'}`);
  ...
  private calculateSimpleScore(event: EventForRecommendation, userBehavior: UserBehavior): number {
    let score = 0;
    let scoringDetails: string[] = [];

    // 🧠 DEBUG: Log user behavior context (only for first event to avoid spam)
    if (Math.random() < 0.05) { // Only 5% chance to log user context
      console.log('👤 USER BEHAVIOR CONTEXT for AI Scoring:');
      console.log(`   🔍 Search Queries: [${userBehavior.searchQueries.slice(-3).join(', ')}] (${userBehavior.searchQueries.length} total)`);
      console.log(`   🏷️  Preferred Categories: [${userBehavior.preferences?.categories?.join(', ') || 'none'}]`);
      console.log(`   📍 Preferred Locations: [${userBehavior.preferences?.locations?.join(', ') || 'none'}]`);
      console.log(`   💰 Price Range: ${userBehavior.preferences?.priceRange ? `${userBehavior.preferences.priceRange.min.toLocaleString()}-${userBehavior.preferences.priceRange.max.toLocaleString()}đ` : 'any'}`);
      console.log(`   👀 Recent Views: ${userBehavior.viewedEvents.length} events`);
      console.log(`   👆 Recent Clicks: ${userBehavior.clickedEvents.length} events`);
      console.log(`   ⚡ Engagements: ${userBehavior.eventEngagement?.length || 0} deep interactions`);
      console.log(`   ════════════════════════════════════════`);
    }
    ...
    if (searchText) {
      ...
      if (searchWords.length > 0) {
        ...
        scoringDetails.push(`Keywords (${keywordMatches}/${searchWords.length}): +${((keywordScore + exactBonus) * 100).toFixed(1)}%`);
      }
    }

    // 2. CATEGORY PREFERENCE (0-25% bonus) - Balanced preference boost
    if (event.category && userBehavior.preferences.categories.length > 0) {
      const categoryFrequency = userBehavior.preferences.categories.filter(cat => 
      ...
      if (categoryFrequency > 0) {
        ...
        score += categoryScore;
        scoringDetails.push(`Category match (${event.category}): +${(categoryScore * 100).toFixed(1)}%`);
      }
    }

    // 3. LOCATION PREFERENCE (0-20% bonus) - Location affinity
    if (event.location && userBehavior.preferences.locations.length > 0) {
      const locationFrequency = userBehavior.preferences.locations.filter(loc => {
        const normalizedEventLoc = event.location?.toLowerCase().replace(/\s+/g, '') || '';
        const normalizedPrefLoc = loc.toLowerCase().replace(/\s+/g, '');
        return normalizedEventLoc.includes(normalizedPrefLoc) || 
      ...
      if (locationFrequency > 0) {
        ...
        score += locationScore;
        scoringDetails.push(`Location match (${event.location}): +${(locationScore * 100).toFixed(1)}%`);
      }
    }
    ...
    if (event.price && userBehavior.preferences.priceRange) {
      ...
      if (event.price >= min && event.price <= max) {
        ...
        score += priceScore;
        scoringDetails.push(`Price preference (${event.price.toLocaleString()}đ): +${(priceScore * 100).toFixed(1)}%`);
      }
    }

    // 5. RECENT INTERACTION BOOST (0-25% bonus) - Enhanced with engagement depth - KEPT SAME
    const recentViews = userBehavior.viewedEvents.filter(ve => 
      ve.category === event.category || 
      ve.location === event.location ||
      (ve.title && event.title && ve.title.toLowerCase().includes(event.title.toLowerCase().split(' ')[0]))
    );
    
    const recentClicks = userBehavior.clickedEvents.filter(ce => 
      ce.category === event.category ||
      (ce.title && event.title && ce.title.toLowerCase().includes(event.title.toLowerCase().split(' ')[0]))
    );

    // Deep engagement tracking - Most important behavioral signal
    const deepEngagement = (userBehavior.eventEngagement || []).filter(ee => 
      ee.eventId === event.id || // Same event
      ee.category === event.category || // Same category
      (ee.location && event.location && ee.location.includes(event.location.substring(0, 5))) // Similar location
    );
    ...
    if (totalInteractions > 0) {
      ...
      scoringDetails.push(`Basic interactions (${totalInteractions}): +${(basicInteractionScore * 100).toFixed(1)}%`);
    }

    // Deep engagement bonus (much more valuable)
    if (deepEngagement.length > 0) {
      const avgTimeSpent = deepEngagement.reduce((sum, e) => sum + e.timeSpent, 0) / deepEngagement.length;
      const avgDepth = deepEngagement.reduce((sum, e) => sum + e.engagementDepth, 0) / deepEngagement.length;
      ...
      scoringDetails.push(`Deep engagement (${deepEngagement.length}x, ${avgTimeSpent.toFixed(0)}s, ${(avgDepth * 100).toFixed(0)}% depth): +${(totalEngagementBonus * 100).toFixed(1)}%`);
    }
    ...
    if (event.created_at) {
      ...
      if (daysSinceCreated > 30) {
        const freshnessPenalty = Math.min((daysSinceCreated - 30) * 0.001, 0.05); // Max -5%
        score -= freshnessPenalty;
        scoringDetails.push(`Freshness penalty (${Math.round(daysSinceCreated)} days): -${(freshnessPenalty * 100).toFixed(1)}%`);
      }
    }
    ...
    console.log(`   💰 Price: ${event.price ? event.price.toLocaleString() + 'đ' : 'N/A'}`);
    console.log(`   ────────────────────────────────────────`);
    
    return score;
  }

  private generateRecommendationReason(event: EventForRecommendation, userBehavior: UserBehavior): string {
    const reasons = [];
    
    if (event.category && userBehavior.preferences.categories.includes(event.category)) {
