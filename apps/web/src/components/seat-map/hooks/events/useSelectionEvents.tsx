...
export interface SelectionHandlers {
  ...
  selectMultipleRows: (ids: string[], multiSelect: boolean) => void;
  selectMultipleSeats: (ids: string[], multiSelect: boolean) => void;
...
export const useSelectionEvents = () => {
  ...
  const handleSelectionMouseDown = useCallback(
    (canvasCoords: any, e: any) => {
      ...
      if (!multiSelect) {
        clearAllSelections(context, handlers);
      }
    },
    [context, handlers]
  );
  ...
  const handleSelectionMouseUp = useCallback(
    (e: any) => {
      ...
      const multiSelect = e.evt?.ctrlKey || e.evt?.metaKey;
      processSelection(selectionRect, multiSelect, context, handlers);
      resetSelectionState();
    },
    [selectionRect, context, handlers]
  );
...
const processSelection = (
  ...
  if (context.isInAreaMode && context.zoomedArea) {
    processAreaSelection(rect, multiSelect, context, handlers);
  } else {
    processShapeSelection(rect, multiSelect, context, handlers);
  }
};

const processAreaSelection = (
  rect: any,
  multiSelect: boolean,
  context: SelectionContext,
  handlers: SelectionHandlers
  ...
  if (seatsInSelection.length > 0) {
    handlers.selectMultipleSeats(seatsInSelection, multiSelect);
  }

  if (rowsInSelection.length > 0) {
    handlers.selectMultipleRows(rowsInSelection, multiSelect);
  }
};
...
const processShapeSelection = (
  ...
  if (shapesInSelection.length > 0) {
    ...
    if (multiSelect) {
      shapeIds.forEach((id) => {
        if (!context.selectedShapeIds.includes(id)) {
          handlers.selectShape(id, true);
        }
      });
    } else {
      handlers.selectMultipleShapes(shapeIds);
    }
  }
};
...
const isShapeInRectangle = (shape: Shape, rect: any) => {
  // Use existing shape detection logic from useSelectionEvents
