...
export function SmartEventGrid({
  ...
  const { recommendations, getRecommendations, isLoading } =
    useAIRecommendations();
  const { userBehavior } = useUserTracking();
  // smartOrderedEvents represents the ordering produced by AI over the aiPool (or events)
  const [smartOrderedEvents, setSmartOrderedEvents] =
    useState<EventSummary[]>(events);
  ...
  const behaviorCheck = useMemo(
    () => ({
      ...
      waitForAI,
      shouldWaitForAI:
        waitForAI && enableSmartOrdering && hasSignificantBehavior,
    }),
    [
      hasSignificantBehavior,
      enableSmartOrdering,
  ...
  const aiEventsIds = useMemo(
    () => aiEvents.map((e) => e.id).join(","),
    [aiEvents]
  );
  ...
  useEffect(() => {
    if (behaviorCheck.shouldUseAI && aiEventsLength > 0) {
      console.log(
        "[SmartEventGrid] Triggering AI recommendations for user with behavior data (pool size:",
        aiEventsLength,
        ")"
      );
    ...
    aiEventsIds,
    getRecommendations,
    behaviorCheck.shouldUseAI,
  ]); // Remove aiEvents from dependencies
