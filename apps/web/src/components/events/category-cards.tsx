...
interface CategoryCardProps {
  title: string;
  value: string; // Internal category value for filtering
  imageUrl: string;
}

const CategoryCard: React.FC<CategoryCardProps> = ({ title, value, imageUrl }) => {
  const router = useRouter();
  
  return (
    <div
      onClick={() =>
        router.push(`/events?category=${encodeURIComponent(value)}`)
      }
...
export default function CategoryList() {
  ...
  const categories = [
  {
    ...
    value: "Entertainment", // Internal value for filtering
    imageUrl:
