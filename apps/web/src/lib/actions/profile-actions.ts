...
import { z } from "zod";
import { getFullUserProfile, updateProfile, updateAvatarUrl } from "@vieticket/services/profile"
...
export async function updateProfileAction(
    ...
    const organizerData = organizerDetails
        ? {
              ...
              foundedDate: organizerDetails.foundedDate ? new Date(organizerDetails.foundedDate) : null,
...
export async function getProfileAction() {
    ...
    if (!session?.user?.id) {
        return { success: false, message: "Unauthorized: You must be logged in.", data: null };
    }
