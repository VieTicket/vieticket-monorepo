...
import type {
  ...
  CompressedTicketPayload,
  CompressedSignedData,
} from "./types";
...
function decompressPayload(
  compressed: CompressedTicketPayload
): TicketValidationPayload {
  return {
 ...
 */
export function decodeTicketQRData(
  qrData: string
): TicketValidationPayload | null {
  try {
    ...
    const [compressedPayload, signature] = unpack(
      binaryData
    ) as CompressedSignedData;

    // Get public key from environment (hex string)
    const publicKeyHex =
      process.env.NEXT_PUBLIC_TICKET_SIGNING_PUBLIC_KEY ||
      import.meta.env.VITE_TICKET_SIGNING_PUBLIC_KEY ||
      process.env.TICKET_SIGNING_PUBLIC_KEY;
    if (!publicKeyHex) {
      console.error(
        "*_PUBLIC_TICKET_SIGNING_PUBLIC_KEY environment variable not set"
      );
      return null;
    }
